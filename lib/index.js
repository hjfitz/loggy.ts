...
class Logger {
  ...
  write(filename) {
    // create the file
    const newFilename = `${filename || ''}${filename ? '-' : ''}${generateTimeStamp()}.log`;
    const writeTo = path.join(process.cwd(), newFilename);
    if (!fs.existsSync(writeTo)) fs.writeFileSync(writeTo, `Begin log ${generateTimeStamp()}`);

    this.writeToFile = true;
    this.filePath = writeTo;
